	...
	var moleRantingSearch = {
	  ...
	  createTween: function () {
	    ...
	    var tween = new mojs.Tween({
	      ...
	      onUpdate: function (p) {
	        mojs.h.style(_this.moleEl, "transform", "skewX(" + 9 * bodySkewC(p) + "deg) scale(" + (1 - 0.01 * bodySkewC(p)) + ", " + (1 + 0.01 * bodySkewC(p)) + ") translateX(" + -7 * bodySkewC(p) + "px) rotateY(" + -10 * bodySkewC(p) + "deg)");

	        mojs.h.style(_this.moleHandEl, "transform", "rotate(" + (33 + 60 * bodySkewC(p)) + "deg) scaleY(" + (1 + 0.07 * bodySkewC(p)) + ")");
